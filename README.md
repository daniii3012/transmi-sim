...
<details>
<summary><b>Las siete cosas que se pueden hacer dentro</b></summary>
...
| Análisis | Hallazgo |
|---|---|
| [Tipo de bus por servicio](docs/TIPOS_DE_BUS_20260912.md) | La etiqueta que lleva escrita cada vehículo separa articulados, biarticulados y duales; los 108 servicios con lecturas usan una sola familia. Retira el criterio de longitud, que erraba en 25 de 76 |
...
| [La operación medida](docs/OPERACION_MEDIDA_20260912.md) | Ocho millones de pares de lecturas: la forma del corredor no cambia con la hora ni el tipo de día, el 73 % de la detención es cola de andén, y una manifestación en la Calle 80 queda excluida del campo |
| [La velocidad la pone el lugar](docs/VELOCIDAD_POR_LUGAR_20260912.md) | Cada trecho de corredor tiene su velocidad medida, de 10 a 44 km/h según dónde. Un bus solo se detiene por cola de andén o por rojo; lo demás se gasta rodando despacio. El apiñamiento, aislado del lugar, vale un 5–8 % |
...
- Salidas del GTFS, por tipo de día, y el tiempo entre paradas medido en el 97 % de los tramos.
- Tipo de bus de 108 servicios, leído de la flota que los atiende.
- Velocidad de cada trecho de corredor, en el 98 % de sus cubetas de 100 m.
...
- Los servicios sin lecturas de flota usan el articulado de referencia.
- Frecuencias fuera del horario publicado, refuerzos en pico y reparto por sentido.
- La velocidad medida se aplica a todas las horas y tipos de día; se comprobó que la forma del corredor no cambia entre franjas.
...
- El 71 % de vehículos troncales en esa familia un sábado conviene contrastarlo con un conteo
  oficial de flota.
- La velocidad medida es del lugar y no distingue quién pasa: un expreso es más rápido que el
  promedio del trecho que cruza, y hoy eso lo compensa el factor por tramo.
- La flota en servicio simulada queda en 0,90 de la observada, porque la cuenta real incluye el bus
  que ya terminó su viaje y sigue reportando en el patio.
- La vigencia publicada del catálogo terminó el 19 de septiembre de 2026 y el escenario se queda en
  esa foto: lo observado después no entra hasta que se vuelva a medir.
...
| `data` | `curated` decisiones revisables · `processed` auditorías. Las instantáneas crudas no se versionan por tamaño; cada dato curado guarda el hash de aquella con la que se construyó |
| `docs` | Documentación activa y los análisis enlazados arriba |
| `tests`, `app/tests` | Pruebas Python y Node |

Ninguna prueba toca la red. Todas corren con el Python del sistema salvo
`test_build_speed_field.py`, que importa la proyección geográfica y necesita el intérprete con
shapely y pyproj. Las herramientas que derivaron
`fleet_types.json`, `speed_field.json` y los tiempos observados quedan en `tools` como documentación
del método: necesitan las lecturas en crudo, que no viajan con el repositorio. `web/transmi2d` es
solo un enlace de compatibilidad con la primera prueba.

...
El lecturas de posición de la flota publica, por vehículo, la etiqueta de flota que el bus lleva pintada
...
las lecturas es `K1657` en la calle, y `E0067` es `E67`. Las series de cuatro cifras (A, M, S, K, U,
...
# Sale de comparar identificador y etiqueta en el lecturas de posición, vehículo por vehículo.
...
def construir(carpeta):
    ...
    return {
        'schema_version': 1,
        'observed_from': observadas[0] if observadas else None,
        'method': ('Tipo por servicio a partir de la etiqueta de flota de las lecturas de posición de la flota; '
                   'ver docs/TIPOS_DE_BUS_20260912.md'),
        'sources': {'routes_sha256': huella, 'feed': 'Lecturas de posición de la flota'},

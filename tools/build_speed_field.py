...
alimentador, y ponía la espera donde no está: en los últimos 440 m antes de la estación.
...
espera que no son suyos—. El alimentador trae la parada a la que cada vehículo se dirige: si está
...
Lo que esto no es. No es un aforo de tráfico ni una medida de congestión por hora: es un día, sábado,
de 14:00 a 23:30, aplicado por ahora a todas las horas y tipos de día. El reparto entre rodar y estar
quieto arrastra además el sesgo conocido del alimentador —un vehículo que no refresca repite posición
...
def construir(carpeta, servicios):
    ...
    return {
        ...
        'observed_days': [f'{d[:4]}-{d[4:6]}-{d[6:]}' for d in dias],
        'assumptions': {

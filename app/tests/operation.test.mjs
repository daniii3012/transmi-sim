...
test('Stretch times come from the street when they were measured, and the switch puts them back',()=>{
 const horario=JSON.parse(fs.readFileSync(new URL('../dist/schedule.json',import.meta.url)));
 const perfiles=JSON.parse(fs.readFileSync(new URL('../dist/speed_profiles.json',import.meta.url)));
 const datos={...source,schedule:horario,speed_profiles:perfiles};
 const conMedida=new Operation(datos,{}),sinMedida=new Operation(datos,{params:{observedRunning:false}});
 assert.ok(conMedida.stats().observedSegments>1000,`usa los tiempos medidos: ${conMedida.stats().observedSegments}`);
 assert.equal(sinMedida.stats().observedSegments,0,'con el interruptor apagado vuelve al horario publicado');
 // Lo medido es más corto que lo publicado, así que el viaje dura menos y ninguno se alarga.
 const mediana=op=>{const v=op.trips.filter(t=>t.start>=DAY&&t.start<2*DAY).map(t=>t.end-t.start).sort((a,b)=>a-b);return v[Math.floor(v.length/2)];};
 assert.ok(mediana(conMedida)<mediana(sinMedida),'el viaje medido dura menos que el publicado');
 assert.ok(mediana(conMedida)>mediana(sinMedida)*.7,'pero no se desploma');
 // Las dos columnas tienen la misma forma, así que el motor no distingue de dónde viene el número.
 for(const reg of Object.values(horario.routes)){
  if(!reg.observed)continue;
  assert.equal(reg.observed.length,reg.segments.length);
  for(const fila of reg.observed)if(fila){assert.equal(fila.length,5);assert.ok(fila.every(x=>x>0));}
 }
});
